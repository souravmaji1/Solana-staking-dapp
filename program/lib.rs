...
declare_id!("6k7uKTB4rXtENAR9tyhhtagLZ6PJxNU1DJcvinSN1JQe");
...
pub mod solana_staking_blog {
    ...
    pool_info.staking_token = ctx.accounts.staking_token.key();
    pool_info.reward_token = ctx.accounts.reward_token.key(); // Add this line
    ...
    mint: ctx.accounts.reward_token.to_account_info(), // Use reward_token instead of staking_token
...
let cpi_accounts = MintTo {
    ...
    // Calculate elapsed hours since deposit
     let elapsed_slots = clock.slot - user_info.deposit_slot;
     let elapsed_seconds = elapsed_slots * 1; // Convert to seconds
     let elapsed_hours = elapsed_seconds / 3600; // Convert to hours

    // Calculate reward based on the formula
     let reward_rate_per_hour = 0.1; // Reward rate per hour per staked token
     let reward = (user_info.amount as u128)
        .checked_mul(reward_rate_per_hour as u128)
        .unwrap()
        .checked_mul(elapsed_hours as u128)
        .unwrap()
...
pub struct Initialize<'info> {
    ...
    #[account(mut)]
    pub reward_token: InterfaceAccount<'info, Mint>, // Add this line
    #[account(mut)]
...
impl PoolInfo {
    pub const LEN: usize = 32 + 8 + 8 + 32 + 32;
}